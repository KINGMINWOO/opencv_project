...
//
//    // 왼쪽 눈과 오른쪽 눈의 객체 생성, 파라미터는 프레임 수
...
//            }
//            //if (i == 0) { // 왼쪽 눈 깜빡임 처리
//            //    left_eye_detector.checkBlink(ok); // 'ok' 변수(true/false)를 직접 전달
...
//            //}
//            //else { // 오른쪽 눈 깜빡임 처리
//            //    right_eye_detector.checkBlink(ok); // 'ok' 변수(true/false)를 직접 전달
...
int main()
{
    ...
    while (true) {
        ...
        for (const Rect& f : faces) {
            ...
            if (eyes.size() == 1) {
                if (!isLeftSide) { // 왼쪽 눈 깜빡임 처리
                    left_eye_detector.checkBlink(false); // 'ok' 변수(true/false)를 직접 전달
                    if (left_eye_detector.isBlinking()) {
                        std::cout << "LEFT CLICK!" << std::endl;
                        putText(frame, "LEFT CLICK!", Point(50, 80), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 255, 0), 2);
                        left_eye_detector.reset();
                    ...
                    right_eye_detector.checkBlink(false); // 'ok' 변수(true/false)를 직접 전달
                    if (right_eye_detector.isBlinking()) {
                        std::cout << "RIGHT CLICK!" << std::endl;
                        putText(frame, "RIGHT CLICK!", Point(50, 120), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 0, 255), 2);
                        right_eye_detector.reset();
